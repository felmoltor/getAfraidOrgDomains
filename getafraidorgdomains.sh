...
for i in $(seq 1 $NPAGES); do curl -s -o "$OUTPUTFOLDER/tmp/"afraidorg.$i.html https://freedns.afraid.org/domain/registry/page-$i.html; done
...
for i in $(seq 1 $NPAGES); do
    modulo=$(($i%10))
    if [ $modulo == 0 ];then
        echo "Parsing file $OUTPUTFOLDER/tmp/afraidorg.$i.html of $NPAGES"
    fi
